...
The product owner accepted the MVP scope and separate-web direction on 2026-08-18. The server-side foundation was implemented on 2026-08-19: migration 022, shared public-visibility policy, reporting/blocking/appeal contracts, reversible moderation and curation actions, safe extraction recovery, and append-only audit history. The focused `admin/` interface and mobile self-service report, block, unblock, status, and appeal controls were completed by 2026-08-20. Operator actions remain isolated from the consumer bundle. Operational details are in `ADMIN-MODERATION-RUNBOOK.md`.
