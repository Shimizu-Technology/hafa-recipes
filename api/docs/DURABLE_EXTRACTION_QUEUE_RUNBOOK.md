...
2. From the production service environment, run:
   ...
   ```bash
   PYTHONPATH=. python migrations/018_add_durable_extraction_jobs.py
   ```
...
Do not deploy the worker code before migration 018. Startup intentionally fails
...
3. Leave migration 018 and job history in place. The migration is expand-only;
